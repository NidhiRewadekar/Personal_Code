...
public class c4_m5 
{
	public static char whose_move (char[][] valid_board_declare)   // method declaration	
	{	
	char turn = 0;
	int r_count=0;
	int y_count=0;
	
		if(valid_board(valid_board_declare)==false) //works correctly, if board is wrong (invalid) then '.' should be returned
		{
			turn='.';
		...
		if(r_count+y_count==42)  // works correctly, if the board is drawn (neither player can make a move) then '.' should be returned
		{
			turn='.';
		...
		if(r_count==y_count) // works correctly, if it is the red players go then 'R' should be returned
		{
			turn='R';
			System.out.println("Next turn:" +turn);
			return turn;
		...
		if(r_count==y_count+1) // works correctly, if it is the yellow players go then 'Y' should be returned
		{
			turn='Y';
			System.out.println("Next turn:" +turn);
			return turn;
		    ...
		    {	
				...
				for(int i=0;i<valid_board_declare.length;++i)   // loops through the number of rows present on the board
				{
					for(int j=0;j<valid_board_declare[i].length;++j)    // loops through the number of columns present on the board		
					{
						...
						if(c=='R')  // checks if the board has a red token, works correctly
						{
							r_count++; // increases total number of red tokens
						}
						
						if (c=='Y')  // checks if the board has a yellow token, works correctly
						{
							y_count++; // increases total number of yellow tokens
						}
					
						if(valid_board_square(c)==false)   // to check for invalid characters, works
						{
				...
				System.out.println("y_count: " + y_count);
						...
						if((r_count != y_count+1) && (r_count!=y_count))   // check that red greater than 1 or equal to yellow, works
                            {
						...
						for(int a=0;a<=valid_board_declare.length-1;++a)  // loops through the number of rows
						{
							for(int b=0;b<valid_board_declare[a].length;++b)   // loops through the number of columns
							{
								...
								if ((a>=0) && (a<valid_board_declare.length-1)) // works
								{
	...
	private static void PrintBoard(char board[][])  //prints the entire board
	{	
