...
public class c4_m2 
{
	...
	{	//test 1	
		...
		if (valid_board_declare!=null)  // checks if the array is not null, works correctly
		{
		    ...
		    {	
				...
				for(int i=0;i<valid_board_declare.length;++i)   // loops through the number of rows present on the board
				{
					for(int j=0;j<valid_board_declare[i].length;++j)    // loops through the number of columns present on the board		
					{
						...
						{
							r_count++; // increases total number of red tokens, works correctly
							//System.out.println("Red tokens:" + r_count);
						}
						
						if (c=='Y')  // checks if the board has a yellow token
						{
							y_count++; // increases total number of yellow tokens, works correctly
							//System.out.println("Yellow tokens:" + y_count);
						}
						
						//System.out.print(c);  // prints the board
					
						if(valid_board_square(c)==false)   // to check for invalid characters, works correctly
						{
				...
				System.out.println("y_count: " + y_count);
						...
						if((r_count != y_count+1) && (r_count!=y_count))   // check that red greater than 1 or equal to yellow, works correctly
                            {
							 System.out.println("R count is more than Y+1 count / R != Y so not checking gravity");    
						...
						for(int a=0;a<=valid_board_declare.length-1;++a)  // loops through the number of rows
						{
							...
							for(int b=0;b<valid_board_declare[a].length;++b)   // loops through the number of columns
							{
								...
								if ((a>=0) && (a<valid_board_declare.length-1))  // works correctly
								{
									e = (char) valid_board_declare[a+1][b];								
								}

							//	if d= yellow or red & e='.'
								//return false
								
								//With defying gravity, you need to check if the square below the square you are checking is blank
								// create an if statement inside the loop where column stays the same, row changes by +1 and check for '.'

							System.out.print(d);   // prints the entire board
								
							if((d=='R') ||(d=='Y'))  // either a red or yellow token works correctly
								{		
								//System.out.print("xxx");
									if(e=='.')  // checks if there is an empty space below a red or yellow token, works correctly
									{
						...
						}  // end of nested for loop, works correctly
						//return true;				
		    } 
			else  // works correctly
			{
				System.out.println("Incorrect Board Size"); 
				return false; // incorrect board size  **check if required	
